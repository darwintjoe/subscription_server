...
ALTER TABLE users ADD COLUMN roles_json TEXT NOT NULL DEFAULT '["reseller"]';
...
DROP TABLE IF EXISTS payment_intents_rebuild;
CREATE TABLE payment_intents_rebuild (
  id TEXT PRIMARY KEY,
  order_id TEXT NOT NULL UNIQUE,
  actor_user_ref TEXT,
  actor_role TEXT,
  actor_email TEXT,
  channel TEXT NOT NULL DEFAULT 'reseller' CHECK(channel IN ('api', 'reseller', 'admin')),
  flow_type TEXT NOT NULL DEFAULT 'reseller_code' CHECK(flow_type IN ('direct_subscribe','reseller_code','bulk_printed_card')),
  duration_code TEXT NOT NULL DEFAULT '12_months' CHECK(duration_code IN ('6_months','12_months')),
  payment_method TEXT NOT NULL DEFAULT 'qris' CHECK(payment_method IN ('qris', 'card')),
...
INSERT INTO payment_intents_rebuild (
  id, order_id, provider, payment_method, provider_ref, status, amount_minor, currency, created_at, updated_at
)
SELECT id, order_id, provider, method, provider_ref, status, amount_minor, currency, created_at, updated_at
FROM payment_intents;
DROP TABLE payment_intents;
ALTER TABLE payment_intents_rebuild RENAME TO payment_intents;
